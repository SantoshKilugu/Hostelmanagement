...
const AdminOutpass = () => {
  ...
  const handleVerifyPinkPass = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        await updateGatepass(rollNo,expectedOutTime);
  ...
  const updateGatepass = async (rollNo, expectedOutTime) => {
    try {
      const response = await fetch(`http://localhost:3300/update-outpass-issue`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({ roll_no: rollNo,expected_out_time: expectedOutTime }),
      });
      ...
      if (!response.ok) {
        ...
        setError1( `You have issued pink pass on ${currentDate1} with expected out time ${expectedOutTime}`);
  ...
  const handleVerifyFingerprint = async () => {
    ...
    try {
        ...
        if (data && Object.keys(data).length > 0) {
            ...
            await updateGatepass(data.studentId,expectedOutTime); // Use data.studentId
        } else {
            alert("No user found.");
        }
    } catch (error) {
...
{(!error) && fingerprintData && (
  <div style={{ marginTop: '20px', textAlign: 'center' }}>
    ...
    <button className="bg-gray-900 text-white font-semibold py-2 px-4 rounded hover:bg-gray-700 transition duration-200 " onClick={generateOutpassPDF}>
      Print Outpass
    </button>
  </div>
)}
